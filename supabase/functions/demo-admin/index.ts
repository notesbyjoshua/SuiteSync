...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'create_suite') {
      const { error } = await client.from('suites').insert({ name: body.name, session: body.session || null, capacity: body.capacity, created_by: admin.user_id });
      if (error) throw error;
